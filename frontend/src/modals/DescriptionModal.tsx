...
function DescriptionModal({
  ...
  const updateDescription = async (newDesc: string) => {
    try {
      socket?.emit(
        "update",
        { id: boardId, changes: { content: newDesc } },
  ...
  const throttledUpdateDescription = useCallback(throttle(updateDescription, 500), []);
  ...
  return (
    <>
      <Button
        onClick={onOpener}
        variant={"outline"}
        className="mx-5 dark:bg-[#161a1d] dark:border-[#1C2025] dark:hover:bg-[#22272b]"
      >
        Description
      </Button>
      <Modal
        ...
        size="full"
        className="dark:text-white flex z-[9999999]"
        backdrop="blur"
        radius="lg"
        isOpen={isOpen}
                      ...
                      config={{
                        ...
                        events: {
                          contentChanged: function () {
                            const editor = this as unknown as FroalaEditor.FroalaEditor;
                            const newContent: string = editor.html.get();
                            setDesc(newContent);
                            throttledUpdateDescription(newContent);
                          },
